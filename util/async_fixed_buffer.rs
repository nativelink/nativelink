...
fn wake(waker: &mut Option<Waker>) {
    waker.take().map(|w| {
        // println!("HAS WAKER, so Waking now");
        w.wake();
    });
}
    ...
    fn poll_read(
        ...
        if num_read <= 0 {
            if me.received_eof.load(Ordering::Relaxed) {
                // println!("Waking READ because EOF RECEIVED");
                wake(&mut waker);
                return Poll::Ready(Ok(()));
            } else if me.did_shutdown.load(Ordering::Relaxed) {
                // println!("Waking READ because Error in disconnect");
                wake(&mut waker);
        ...
        if num_read <= 0 {
            // println!("Parking READ because no data");
            park(&mut waker, &cx);
            return Poll::Pending;
        }
        // println!("Waking READ Got {} byes", num_read);
        wake(&mut waker);
        Poll::Ready(Ok(()))
    }
}
...
impl<T: AsMut<[u8]> + AsRef<[u8]>> AsyncWrite for AsyncFixedBuf<T> {
    fn poll_write(self: Pin<&mut Self>, cx: &mut Context<'_>, buf: &[u8]) -> Poll<Result<usize, std::io::Error>> {
        ...
        if me.did_shutdown.load(Ordering::Relaxed) {
            // println!("Waking WRITE because shutdown");
            wake(&mut waker);
            return Poll::Ready(Err(std::io::Error::new(
                std::io::ErrorKind::BrokenPipe,
        ...
        if buf.len() == 0 {
            ...
            // println!("poll_write waking due to eof received");
            wake(&mut waker);
        ...
        match me.inner.writable() {
            Some(writable_slice) => {
                ...
                // println!("poll_write waking because wrote {} bytes", write_amt);
                wake(&mut waker);
            ...
            None => {
                ...
                // println!("ABOUT TO ENTER   {} / {}", (me.inner.len() as f32), (me.inner.capacity() as f32));
                if (me.inner.len() as f32) / (me.inner.capacity() as f32) < 0.25 {
                    // println!("ABOUT TO ENTER   {} / {}", (me.inner.len() as f32), (me.inner.capacity() as f32));
                    me.inner.shift();
                    // println!("ABOUT TO ENTER 2 {} / {}", (me.inner.len() as f32), (me.inner.capacity() as f32));
                ...
                // println!("Parking WRITE because not writable");
                park(&mut waker, &cx);
                Poll::Pending
            }
        }
    ...
    fn poll_flush(self: Pin<&mut Self>, cx: &mut Context<'_>) -> Poll<Result<(), std::io::Error>> {
        ...
        if self.inner.is_empty() {
            // println!("Flush waking");
            wake(&mut waker);
            return Poll::Ready(Ok(()));
        }
        // println!("Parking FLUSH because not empty");
        park(&mut waker, &cx);
        Poll::Pending
    }

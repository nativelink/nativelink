...
// Default block size to chunk into
    ...
    fn get_part<'a>(
        ...
        Box::pin(async move {
            ...
            let read_fut = async move {
                ...
                let mut uncompressed_data = vec![0u8; get_maximum_output_size(self.config.block_size as usize)];
                ...
                while frame_type != FOOTER_FRAME_TYPE {
                    error_if!(
                        ...
                        "Expected frame to be BODY or HEADER in compression store, got {} at {}",
                        frame_type,
                        chunks_count
                    );
                    rx_data_buf.clear();
